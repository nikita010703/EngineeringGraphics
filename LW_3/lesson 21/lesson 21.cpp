...
public:
    ...
    bool Init() {
        ...
        m_pTexture = new Texture(GL_TEXTURE_2D, "C:/tmp/test3.png");
        if (!m_pTexture->Load())
            return false;
    ...
    virtual void RenderSceneCB() {
        ...
        SpotLight sl[2];
        sl[0].DiffuseIntensity = 7.0f;
        sl[0].Color = Vector3f(sinf(m_scale) / 2 + 0.5f,
            sinf(m_scale / 2) / 2 + 0.5f,
            sinf(m_scale / 3) / 2 + 0.5f);
        sl[0].Position = Vector3f(0.0f, 1.0f, 0.0f);
        sl[0].Direction = Vector3f(sinf(m_scale / 2) * 2, -1.0f, cosf(m_scale / 2) * 2);
        sl[0].Attenuation.Linear = 0.1f;
        sl[0].Cutoff = 20.0f;

        sl[1].DiffuseIntensity = 5.0f;
        sl[1].Color = Vector3f(0.0f, 0.5f, 0.5f);
    ...
    virtual void KeyboardCB(unsigned char Key, int x, int y) {
        ...
        case 'a':
            m_directionalLight.AmbientIntensity += 0.05f; break;

        case 's':
            m_directionalLight.AmbientIntensity -= 0.05f; break;

        case 'z':
            m_directionalLight.DiffuseIntensity += 0.05f; break;
